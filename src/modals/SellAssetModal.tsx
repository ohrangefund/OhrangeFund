...
import { X, ChevronRight, CalendarDays, TrendingDown } from 'lucide-react-native';
import { useTranslation } from 'react-i18next';
import { useTheme } from '@/context/ThemeContext';
import { useAuth } from '@/context/AuthContext';
import { sellAsset } from '@/api/investmentTransactions';
...
export function SellAssetModal({
  visible, investmentAccountId, assets, accounts, prices, onClose,
}: Props) {
  const { colors } = useTheme();
  const { user } = useAuth();
  const { t } = useTranslation();

  const ownedAssets = assets.filter((a) => a.quantity > 0);
  ...
  useEffect(() => {
    ...
    const cached = prices[selectedAsset.ticker];
    if (cached) { setCurrentPrice(cached); return; }
  ...
  const maxValueCents = selectedAsset && currentPrice
    ? Math.round(selectedAsset.quantity * currentPrice)
    : null;
  ...
  async function handleSubmit() {
    ...
    if (maxValueCents !== null && cents > maxValueCents + 1) {
      setError(t('investments.insufficientQuantity')); return;
    }
    ...
    try {
      ...
      if (e?.message === 'INSUFFICIENT_QUANTITY') {
        setError(t('investments.insufficientQuantity'));
      } else {
        setError(t('investments.errorSell'));
      }
    } finally {
      setLoading(false);
    }
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={{ flex: 1 }}>
        <View style={[styles.container, { backgroundColor: colors.background }]}>
          <View style={[styles.header, { borderBottomColor: colors.border }]}>
            <Text style={[styles.title, { color: colors.text }]}>{t('investments.sellTitle')}</Text>
          ...
          <ScrollView contentContainerStyle={styles.body} showsVerticalScrollIndicator={false}>
            ...
            >
              {selectedAsset ? (
                <View style={styles.selectorContent}>
                  <View style={[styles.selectorIcon, { backgroundColor: colors.expense + '22' }]}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.selectorSub, { color: colors.textSecondary }]}>
                      {t('investments.held')}: {selectedAsset.quantity % 1 < 0.0001 ? selectedAsset.quantity.toFixed(0) : selectedAsset.quantity.toFixed(4)} un
                      {maxValueCents !== null ? ` · ${formatCurrency(maxValueCents)}` : ''}
                    </Text>
                  </View>
                </View>
              ) : (
            ...
            {/* Credit account */}
            <Text style={[styles.label, { color: colors.textSecondary }]}>{t('investments.creditAccount')}</Text>
            <Pressable
            ...
            {estimatedQty !== null && (
              <View style={[styles.estimateBox, { backgroundColor: colors.expense + '14' }]}>
                <Text style={[styles.estimateText, { color: colors.expense }]}>
          ...
          <View style={[styles.footer, { borderTopColor: colors.border }]}>
            <Pressable
              ...
              disabled={loading}
              style={({ pressed }) => [styles.submitBtn, { backgroundColor: colors.expense, opacity: pressed ? 0.8 : 1 }]}
            >
              {loading
                ? <ActivityIndicator color="#fff" />
                : <Text style={styles.submitText}>{t('investments.sellBtn')}</Text>
              }
            </Pressable>
          </View>
        </View>
        ...
        <View style={[styles.container, { backgroundColor: colors.background }]}>
          ...
          <ScrollView contentContainerStyle={styles.body}>
            ...
            {ownedAssets.map((asset) => {
              const val = prices[asset.ticker]
                ? formatCurrency(Math.round(asset.quantity * prices[asset.ticker]))
                : null;
              return (
                <Pressable
                  key={asset.id}
                ...
                >
                  <View style={{ flex: 1 }}>
                    ...
                    <Text style={[styles.assetName, { color: colors.textSecondary }]}>
                      {asset.quantity % 1 < 0.0001 ? asset.quantity.toFixed(0) : asset.quantity.toFixed(4)} un
                    </Text>
                  </View>
                  {val && <Text style={[styles.assetValue, { color: colors.text }]}>{val}</Text>}
                </Pressable>
              );
            })}
          </ScrollView>
...
const styles = StyleSheet.create({
  ...
  assetTicker: { fontSize: 15, fontWeight: '700' },
